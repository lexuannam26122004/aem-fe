...
import { useGetByIdSupplierQuery, useUpdateSupplierMutation } from '@/services/SupplierService'
...
function Page() {
    ...
    const { data: supplierResponse, isLoading: isLoadingSupplier, refetch } = useGetByIdSupplierQuery(id)
    ...
    useEffect(() => {
        if (supplier) {
            setAvatarPath(supplier.avatarPath)
        ...
        <Box sx={{ width: '100%', maxWidth: '1100px', margin: '0 auto', display: 'flex', gap: '24px' }}>
            ...
            >
                <Typography sx={{ fontWeight: 'bold', fontSize: '22px', color: 'var(--text-color)' }}>
                    {t('COMMON.SUPPLIERS.UPDATE_SUPPLIER')}
                </Typography>
