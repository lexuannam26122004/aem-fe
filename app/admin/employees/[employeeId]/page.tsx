...
import { useUpdateEmployeeMutation, useGetByIdEmployeeQuery } from '@/services/EmployeeService'
...
function Page() {
    ...
    const { data: roleResponse, isLoading: isLoadingRole } = useGetAllRolesQuery({
        pageNumber: 1,
        pageSize: 100,
        sortBy: 'Name',
        isDescending: false
    ...
    const { data: infoResponse, isLoading: isLoadingInfo } = useGetByIdEmployeeQuery(String(id))
    ...
    useEffect(() => {
        if (infoData) {
            ...
            setAvatarPath(infoData.avatar || '')
            setRoles(infoData.roles || [])
            setIsActive(infoData.isActive)
        }
    }, [infoData])
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setIsSubmit(false)
            toast(t('COMMON.EMPLOYEES.UPDATE_EMPLOYEE_SUCCESS'), 'success')
            return true
        } catch (error) {
    ...
    if (isLoadingInfo || isLoadingRole) {
        return <Loading />
    }
