...
import { useLazyGetCartCountQuery } from '@/services/CartService'
import { useLazyGetFavoriteCountQuery } from '@/services/FavoriteService'
...
export default function MainLayout({ children }: { children: ReactNode }) {
    ...
    const [triggerCartCount, { isLoading: isCartCountLoading }] = useLazyGetCartCountQuery()
    const [triggerFavoriteCount, { isLoading: isFavoriteCountLoading }] = useLazyGetFavoriteCountQuery()
    ...
    useEffect(() => {
        if (isAuthenticated) {
            triggerCartCount()
                .unwrap()
                .then(res => dispatch(setCartCount(res.data)))
            triggerFavoriteCount()
                .unwrap()
                .then(res => dispatch(setFavoriteCount(res.data)))
        }
    }, [isAuthenticated])
