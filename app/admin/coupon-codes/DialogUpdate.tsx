...
import { useGetByIdCouponQuery, useUpdateCouponMutation } from '@/services/CouponService'
import { ICouponUpdate } from '@/models/Coupon'
...
function DialogUpdate({ open, handleClose, id }: Props) {
    const { t } = useTranslation('common')
    const [couponCode, setCouponCode] = useState('')
    const [discountType, setDiscountType] = useState<'percentage' | 'fixed'>('percentage')
    const [discountValue, setDiscountValue] = useState<number | undefined>(10)
    const [title, setTitle] = useState<string>('')
    const [couponType, setCouponType] = useState<'product' | 'shipping'>('product')
    ...
    const { data: infoResponse, isLoading: isLoadingInfo } = useGetByIdCouponQuery(id)

    useEffect(() => {
        if (infoResponse) {
            setCouponCode(infoResponse.data?.couponCode)
            setDiscountType(infoResponse.data?.discountType || 'percentage')
            setDiscountValue(infoResponse.data?.discountValue || 10)
            setTitle(infoResponse.data?.title || '')
            setCouponType(infoResponse.data?.couponType || 'product')
            setMinimumOrderValue(infoResponse.data?.minimumOrderValue || 0)
            setMaximumDiscount(infoResponse.data?.maximumDiscount || 10000)
            setUsageLimit(infoResponse.data?.usageLimit)
            setCustomerType(infoResponse.data?.customerType || 'all_customer')
            setActivationDate(dayjs(infoResponse.data?.activationDate).toDate().toDateString())
            setExpiryDate(dayjs(infoResponse.data?.expiryDate).toDate().toDateString())
        }
    }, [infoResponse])
    ...
    const handleSave = async () => {
        ...
        try {
            await updateCoupon({ id, data: body }).unwrap()
            setIsSubmit(false)
            toast(t('COMMON.COUPON.UPDATE_SUCCESS'), 'success')
            return true
        } catch (error) {
                ...
                >
                    ...
                    >
                        <Typography
                            sx={{
                                fontSize: '16px',
                                color: isSubmit && title.trim() === '' ? 'var(--error-color)' : 'var(--text-color)',
                                fontWeight: 'bold',
                                mb: '10px'
                            }}
                        >
                            {t('COMMON.COUPON.COUPON_TITLE')}
                        </Typography>

                        <TextField
                            fullWidth
                            placeholder={t('COMMON.COUPON.ENTER_COUPON_TITLE')}
                            value={title}
                            slotProps={{
                                input: {
                                    sx: {
                    ...
                    >
                        ...
                        >
                            <Select
                                value={couponType}
                                onChange={e => setCouponType(e.target.value as 'product' | 'shipping')}
                                sx={{
                                    width: '100%',
                                    '&:hover .MuiOutlinedInput-notchedOutline': {
                                        borderColor: 'var(--border-color)'
